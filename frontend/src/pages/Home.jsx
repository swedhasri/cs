...
const Home = () => {
    ...
    return (
        <div className="page-container">
            ...
            <section className="relative overflow-hidden">
                ...
                <div className="relative section-container py-24 lg:py-32">
                    <div className="text-center max-w-4xl mx-auto">
                        {/* Badge */}
                        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-cyber-cyan/30 bg-cyber-cyan/5 mb-8 animate-reveal">
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 animate-fade-in-up delay-300">
                            {!isAuthenticated ? (
                                <Link to="/register" className="cyber-button-primary text-lg !px-8 !py-4 hover-lift hover-glow">
                                    Get Started Free
                                </Link>
                            ) : (
                                <Link to="/dashboard" className="cyber-button-primary text-lg !px-8 !py-4 hover-lift hover-glow">
                                    Go to Dashboard
                                </Link>
                            )}
                            <Link to="/tools" className="cyber-button-secondary text-lg !px-8 !py-4 hover-lift hover-border">
                                Explore Tools
                            </Link>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-8 mt-16 animate-fade-in-up delay-400">
                            {[
                                ...
                                <div key={label} className="text-center animate-scale-in hover-scale" style={{ animationDelay: `${0.5 + index * 0.15}s` }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto">
                    {tools.map(({ title, description, icon: Icon, to, gradient, shadow }, index) => (
                        <Link
                            key={to}
                            to={to}
                            className={`glass-card p-6 group hover:${shadow} hover:border-cyber-cyan/30 transition-all duration-700 animate-slide-in hover-lift hover-glow transition-all-smooth`}
                            style={{ animationDelay: `${0.3 + index * 0.2}s` }}
                        >
                            <div className="flex items-start gap-4">
                                <div className={`flex-shrink-0 w-12 h-12 rounded-xl bg-gradient-to-br ${gradient} flex items-center justify-center group-hover:scale-110 transition-all duration-500 animate-float`}>
                                    <Icon className="w-6 h-6 text-white" />
                                </div>
                                <div className="flex-1">
                                    <h3 className="text-lg font-semibold text-cyber-text group-hover:text-cyber-cyan transition-colors duration-500 mb-2">
                                        {title}
                                    </h3>
                                    <p className="text-sm text-cyber-muted leading-relaxed">{description}</p>
                                </div>
                                <HiChevronRight className="w-5 h-5 text-cyber-muted group-hover:text-cyber-cyan group-hover:translate-x-2 transition-all duration-500" />
                            </div>
                        </Link>
                    ))}
                </div>
            ...
            {/* Threat Feed Preview */}
            <section className="section-container">
                <div className="glass-card p-8 sm:p-12 text-center animate-reveal">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyber-red/10 border border-cyber-red/30 mb-6 animate-glow hover-glow">
                    ...
                    <Link to="/threat-feed" className="cyber-button-primary hover-lift hover-glow animate-fade-in-up delay-300 transition-all-smooth">
                        View Threat Feed
                    </Link>
                </div>
            </section>
            ...
            {!isAuthenticated && (
                <section className="section-container pb-24">
                    <div className="relative rounded-2xl overflow-hidden p-8 sm:p-16 animate-slide-in">
                        <div className="absolute inset-0 bg-gradient-to-r from-cyber-blue/20 to-cyber-purple/20 animate-parallax"></div>
                        <div className="absolute inset-0 bg-cyber-card/50 backdrop-blur-sm"></div>
                        <div className="relative text-center">
                            <HiShieldCheck className="w-16 h-16 text-cyber-cyan mx-auto mb-6 animate-float hover-scale" />
                            ...
                            <Link to="/register" className="cyber-button-primary text-lg !px-8 !py-4 hover-lift hover-glow animate-fade-in-up delay-300 transition-all-smooth">
                                Create Free Account
                            </Link>
                        </div>
                    </div>
